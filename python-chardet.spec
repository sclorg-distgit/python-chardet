...
BuildRequires:  %{?pythonscl:%{pythonscl}-}python-setuptools
...
%install
%{?scl:scl enable %{scl} %{?pythonscl} - << \EOF}
python setup.py install -O1 --skip-build --root %{buildroot} --install-scripts=%{_bindir} --install-purelib %{python_sitelib}
%{?scl:EOF}
...
%changelog
* Fri Feb 23 2018 Marek Skalický <mskalick@redhat.com> - 2.3.0-6
- Add -runtime requirement

* Thu Feb 08 2018 Marek Skalický <mskalick@redhat.com> - 2.3.0-5
- Install into rh-mongodb36 prefix

* Wed Dec 20 2017 Marek Skalický <mskalick@redhat.com> - 2.3.0-4
